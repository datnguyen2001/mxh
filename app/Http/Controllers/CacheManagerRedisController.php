...
class CacheManagerRedisController extends Controller
{
        ...
        switch ($action) {
            ...
            case 'delete': {
                ...
                try {
                    ...
                    $redisCached->delete($prefix.$path);
                    return  response()->json([
                        'status' => true,
            ...
            case 'remove_port': {
                //Remove Port by Server
                $server=$path;
                $cacheServer=config('cachepage.CachePage.RedisCacheSerVer.'.$server);
                if (!$cacheServer){
                    return  response()->json([
                        'status' => false,
                    ...
                    Cache::store($cachepage['RedisCacheSerVer'][$cacheServer]??'')->tags(config('cache.prefix'))->flush();
                    return  response()->json([
                        'status' => true,
