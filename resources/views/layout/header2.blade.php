<div class="header__new">
    <div class="header__content page__content">
        <img src="{{asset('image/logo.png')}}" alt="">
        <div class="box-content-link">
            <div class="header__content-center">
                <div class="header__content-center-left">
                    <a href="#" class="link-menu-header link-menu-header-active">
                        <img src="{{asset('image/icon-home.png')}}" alt="">
                    </a>
                    <a href="#" class="link-menu-header">
                        <img src="{{asset('image/icon-video.png')}}" alt="">
                    </a>
                    <a href="#" class="link-menu-header">
                        <img src="{{asset('image/icon-layout.png')}}" alt="">
                    </a>
                    <a href="#" class="link-menu-header">
                        <img src="{{asset('image/icon-cart.png')}}" alt="">
                    </a>
                    <a href="#" class="link-menu-header">
                        <img src="{{asset('image/icon-setting-people.png')}}" alt="">
                    </a>
                </div>
                <div class="header__content-center-right">
                    <a href="#" class="link-login-header">
                        <img src="{{asset('image/icon-fb.png')}}" alt="">
                    </a>
                    <a href="#" class="link-login-header">
                        <img src="{{asset('image/icon-x.png')}}" alt="">
                    </a>
                </div>
            </div>
            <div class="header__content-right">
                <div class="header__search-box">
                    <img src="{{asset('image/icon-search.png')}}" alt="" class="icon-box-search">
                    <input type="text" class="input-header-search" placeholder="Tìm kiếm">
                </div>
                <div class="header__search-box-tablet">
                    <img src="{{asset('image/icon-search.png')}}" alt="" class="icon-box-search">
                </div>
                <div class="box-noti-header">
                    <img src="{{asset('image/icon-noti.png')}}" alt="" class="header-icon-bell">
                </div>
                <div class="box-account-header">
                    <img src="{{asset('image/icon-people.png')}}" alt="" class="header-icon-user">
                </div>
            </div>
        </div>

    </div>
</div>

<div class="header__new-mobile">
    <div class="header__new-mobile-top">
        <img src="{{asset('image/search-header-mobile.png')}}" alt="" class="icon-search-header-mobile">
        <img src="{{asset('image/logo-mobile.png')}}" alt="" class="icon-logo-header-mobile">
        <img src="{{asset('image/tab-header-mobile.png')}}" alt="" class="icon-tab-header-mobile">
    </div>
    <div class="header__new-mobile-bottom">
        <a href="#" class="link-menu-header-mobile">
            <img src="{{asset('image/icon-home.png')}}" alt="">
        </a>
        <a href="#" class="link-menu-header-mobile">
            <img src="{{asset('image/icon-video.png')}}" alt="">
        </a>
        <a href="#" class="link-menu-header-mobile">
            <img src="{{asset('image/icon-layout.png')}}" alt="">
        </a>
        <a href="#" class="link-menu-header-mobile">
            <img src="{{asset('image/icon-cart.png')}}" alt="">
        </a>
        <a href="#" class="link-menu-header-mobile">
            <img src="{{asset('image/icon-setting-people.png')}}" alt="">
        </a>
        <a href="#" class="link-menu-header-mobile">
            <img src="{{asset('image/bell-mobile.png')}}" alt="">
        </a>
        <a href="#" class="link-menu-header-mobile link-menu-header-mobile-end">
            <div class="link-menu-header-mobile-item">
                <img src="{{asset('image/icon-user-mobile.png')}}" alt="">
            </div>
        </a>
    </div>
</div>
