...
@section('Canonical'){{config('siteInfo.site_path').($zoneInfo->ZoneUrl??'')}}@endsection
@section('css')
    @if(!empty($listFocus))
...
@section('content')
                        ...
                        <div class="stream-left">
                            @if(!empty($listNews))
                                <x-template::box-layout3 :listNews="$listNews">
                                    ...
                                    <x-slot name="isPaginate">{!! $pagination ?? '' !!}</x-slot>
    ...
    <script>
        (runinit = window.runinit || []).push(function () {
            $.ajax({
                url: `https://api.hapodigital.com/api/v2/keywords?uri={{config('siteInfo.site_path').($zoneInfo->ZoneUrl??'')}}`,
                type: 'GET',
